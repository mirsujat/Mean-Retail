...
//  var url = 'mongodb://localhost/angular-drwayll';
  var uri = ['mongodb://localhost:27017/drywall'];
...
workflow.on('generateConfigJS', function(){
  async.waterfall([
    function(cb){
      ...
      require('fs').readFile('./config.js', {encoding: 'utf8'}, function(err, data){
    ...
    function(content, cb){
      ...
      var map = {
        'mongodb.uri': workflow.mongo.uri,
        'systemEmail': workflow.admin.email,
        'smtp.credentials.user': smtpEnabled? workflow.smtp.email: '',
        'smtp.credentials.password': smtpEnabled? workflow.smtp.password: '',
        'smtp.credentials.host': smtpEnabled? workflow.smtp.host: ''
      };
      for(var key in map){
